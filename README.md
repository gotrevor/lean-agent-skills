...
| Skill | Use it when |
|-------|-------------|
| [`mathlib-bump`](skills/mathlib-bump/) | A `lake build` goes red after a Mathlib version bump and the errors have **no deprecation warning** to grep for (`unsolved goals`, `made no progress`, `unknown constant`). Symptom → cause → fix catalogs for v4.29.1 → v4.31.0 and the provisional v4.31.0 → v4.32.0 window. |
